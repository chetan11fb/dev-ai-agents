...
function parseArgs(argv) {
  ...
  for (let i=0;i<argv.length;i++) {
    ...
    else if (["--agent","--skill","--plugin","--mcp"].includes(a)) {
      const type=a.slice(2), id=argv[++i];
...
async function writeRemoteFile(destination,url,force) {
  try { await readFile(destination); if (!force) throw new Error(\`File exists: \${destination}. Use --force to overwrite.\`); } catch (e) {
...
async function install(component,root,force) {
  if (component.type==="plugin") {
    ...
    if (!entries.length) throw new Error(\`No files found for plugin \${component.id}\`);
    for (const e of entries) await writeRemoteFile(resolve(root,component.target,e.path.slice(prefix.length)),\`\${RAW_BASE}/\${e.path}\`,force);
...
function list(registry) {
  for (const type of ["agent","skill","plugin","mcp"]) {
    const items=registry.components.filter(x=>x.type===type);
    log(\`\\n\${type.toUpperCase()}S (\${items.length})\`,c.cyan);
    for (const x of items) console.log(\`  \${x.id}  \${c.dim}→ \${x.path}\${c.reset}\`);
  }
}
async function main() {
  ...
  const selected=o.all ? registry.components : o.components.map(x=> {
